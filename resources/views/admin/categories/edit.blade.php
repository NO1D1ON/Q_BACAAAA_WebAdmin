            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" 
                          x-data="{ filePreview: '{{ $category->image_path ? asset('storage/' . $category->image_path) : null }}', fileName: '' }">
                        @csrf
                        @method('PUT')
                        
                        {{-- [MODIFIKASI] Layout diubah menjadi 1 kolom dengan space-y-6 --}}
                        <div class="space-y-6">
                            {{-- Input Nama Kategori --}}
                            <div>
                                <label for="name" class="block font-medium text-sm text-gray-700 pb-2">Nama Kategori</label>
                                <input type="text" name="name" id="name" class="w-full py-2 px-4 rounded-md bg-gray-100 border-gray-300 focus:border-[#28738B] focus:ring focus:ring-[#28738B] focus:ring-opacity-50 transition" value="{{ old('name', $category->name) }}" required>
                            </div>
                            
                            {{-- Pratinjau Gambar --}}
                            <div>
                                <label class="block font-medium text-sm text-gray-700">Pratinjau Gambar</label>
                                {{-- [MODIFIKASI] Ukuran kontainer pratinjau dibuat tetap dengan h-48 --}}
